...
require_once __DIR__ . '/db_schema_deploy_helpers.php';
...
{
    if (empty($_SESSION['admin_db_schema_deploy_token']) || !is_string($_SESSION['admin_db_schema_deploy_token'])) {
        $_SESSION['admin_db_schema_deploy_token'] = bin2hex(random_bytes(16));
    }
    return (string)$_SESSION['admin_db_schema_deploy_token'];
}
...
{
    $token = (string)($post['admin_db_schema_deploy_token'] ?? '');
    return $token !== '' && hash_equals(corebb_admin_schema_deploy_token(), $token);
}
