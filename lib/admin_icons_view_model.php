...
function corebb_admin_icons_token(): string
{
    ...
    if (empty($_SESSION['admin_icons_token'])) {
        $_SESSION['admin_icons_token'] = bin2hex(random_bytes(16));
    }
    return (string)$_SESSION['admin_icons_token'];
}
...
{
    $expected = corebb_admin_icons_token();
    $got = (string)($post['admin_icons_token'] ?? '');
...
function corebb_admin_icons_safe_file_path(string $publicPath): ?string
{
    ...
    $candidate = dirname(__DIR__) . '/' . $safePublicPath;
