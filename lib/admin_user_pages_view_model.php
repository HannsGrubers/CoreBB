...
function corebb_user_pages_token(): string
{
    ...
    if (empty($_SESSION['admin_user_pages_token'])) {
        $_SESSION['admin_user_pages_token'] = bin2hex(random_bytes(16));
    }
    return (string)$_SESSION['admin_user_pages_token'];
}
...
{
    $expected = corebb_user_pages_token();
    $got = (string)($post['admin_user_pages_token'] ?? '');
