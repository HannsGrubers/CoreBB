...
function corebb_admin_edit_profile_token(): string
{
    ...
    if (empty($_SESSION['admin_edit_profile_token'])) {
        $_SESSION['admin_edit_profile_token'] = bin2hex(random_bytes(16));
    }
    return (string)$_SESSION['admin_edit_profile_token'];
}
...
{
    $expected = corebb_admin_edit_profile_token();
    $got = (string)($post['admin_edit_profile_token'] ?? '');
