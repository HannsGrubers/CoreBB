...
{
    return corebb_security_named_token('corebb_install_token');
}
...
{
    return corebb_security_named_token_valid('corebb_install_token', $post, 'install_token');
}
...
{
    $path = corebb_install_root() . DIRECTORY_SEPARATOR . 'lib' . DIRECTORY_SEPARATOR . 'helpers' . DIRECTORY_SEPARATOR . 'install_schema.sql';
    if (!is_file($path)) {
...
{
    require_once corebb_install_root() . '/lib/helpers/auth_password_helpers.php';
...
function corebb_install_model(array $post, array $server): array
{
    ...
    $model = [
        ...
        'requirements' => [
            ...
            ['label' => 'Install schema present', 'ok' => is_file(corebb_install_root() . '/lib/helpers/install_schema.sql')],
