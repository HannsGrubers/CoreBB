...
function corebb_admin_mod_token(): string
{
    ...
    if (empty($_SESSION['admin_moderation_token'])) {
        $_SESSION['admin_moderation_token'] = bin2hex(random_bytes(16));
    }
    return (string)$_SESSION['admin_moderation_token'];
}
...
{
    $expected = corebb_admin_mod_token();
    $got = (string)($post['admin_moderation_token'] ?? '');
