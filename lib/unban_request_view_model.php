...
function corebb_unban_token(): string
{
    ...
    if (empty($_SESSION['unban_request_token'])) {
        $_SESSION['unban_request_token'] = bin2hex(random_bytes(16));
    }
    return (string)$_SESSION['unban_request_token'];
}
...
{
    $expected = corebb_unban_token();
    $got = (string)($post['unban_request_token'] ?? '');
